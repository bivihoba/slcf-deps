...
var readBundleDeps = function(bundleName) {
	var fileContent =  fs.existsSync(bundleDepsPath(bundleName)) ? fs.readFileSync(bundleDepsPath(bundleName)) : undefined;
	var deps = [];
		if (fileContent) {
...
var replaceLocalPaths = function(deps, bundleName) {
	var importStartPattern = '@import "';
	var currentDirectoryImportPattern = '@import "b-';
	var importCurrentDirectory = getBundleLocalPath(bundleName) + '/';

	deps = _.map(deps, function(i) {
		if (s(i).startsWith(currentDirectoryImportPattern)) {
			i = i.replace(importStartPattern,importStartPattern + importCurrentDirectory);
		}
		return i;
	});
